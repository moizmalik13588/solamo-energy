...
export default function SolarPanelsPage() {
  ...
  const updateUrlParams = (brands: string[], min: string, max: string) => {
    ...
    const newUrl = queryStr ? `/solar-panels?${queryStr}` : "/solar-panels";
  ...
  // Flexible filter for Solar Panels (checks if category contains 'solar' or 'panel')
  const solarPanelProducts = allProducts.filter((product) => {
    if (!product || !product.category) return false;
    const cat = product.category.toLowerCase();
    return cat.includes("solar") || cat.includes("panel");
  });
  ...
  const filteredProducts = solarPanelProducts.filter((product) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="container mx-auto px-4 md:px-8 pt-16 pb-10 flex-grow">
        ...
        {/* Heading matching live site ("Solar" in dark, "Panels" in green) */}
        <h1 className="text-4xl sm:text-5xl font-bold mb-10 text-gray-900">
          Solar <span className="text-lime-500">Panels</span>
        </h1>
