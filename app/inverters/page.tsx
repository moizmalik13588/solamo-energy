...
function InvertersPageContent() {
  ...
  // Filter products specifically for the "Inverters" category
  const inverterProducts = allProducts.filter(
    (product) =>
      product &&
      product.category &&
      (product.category.toLowerCase() === "inverters" ||
        product.category.toLowerCase() === "inverter"),
  );
  ...
  const filteredProducts = inverterProducts.filter((product) => {
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="container mx-auto px-4 md:px-8 pt-16 pb-10 flex-grow">
        ...
        <h1 className="text-4xl sm:text-5xl font-bold mb-10 text-gray-900">
          <span className="text-lime-500">Inverters</span>
        </h1>
